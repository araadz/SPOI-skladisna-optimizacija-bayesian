...
def bayesian_optimize_parameters(df, n_calls=15, progress_callback=None):
    ...
    res = gp_minimize(
        ...
        n_calls=n_calls,
        random_state=42,
        callback=[lambda r: progress_callback(len(r.x_iters), n_calls, -r.fun) if progress_callback else None]
    )
